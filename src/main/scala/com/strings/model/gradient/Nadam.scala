...
class Nadam(val epsilon:Double = 1e-6,
 ...
 override def fit(x: BDM[Double], y_train: BDV[Double]):(BDV[Double],Int)  = {
    ...
    for (_ <- 0 to num_iters if flag) {
      ...
      for((mini_X,mini_y) <- batch_data) {
        ...
        val up:BDV[Double] = beta_1*m_t_hat + (1 - beta_1)/(1- scala.math.pow(beta_1,n_iters)) * mini_gradient
        val gradient_nadam = up :/ v_t_hat.map(x => scala.math.sqrt(x) + epsilon)
        weights -= gradient_nadam * lr //不能写成 :*
      }
