...
class AdaptiveGradientDescent(val epsilon:Double = 1e-6,
  ...
  override def fit(x: BDM[Double], y_train: BDV[Double]):(BDV[Double],Int)  = {
    ...
    var n_iters: Int = 0
    var gradient_sum = BDV.zeros[Double](n_features)
    ...
    for (_ <- 0 to num_iters if flag) {
      ...
      for((mini_X,mini_y) <- batch_data) {
        ...
        gradient_sum += mini_gradient :* mini_gradient
        val adj_gradient =  mini_gradient :/gradient_sum.map(x => scala.math.sqrt(x) + epsilon)
        weights = weights - (adj_gradient :* lr) //不能写成 weights - adj_gradient :* lr
      }
