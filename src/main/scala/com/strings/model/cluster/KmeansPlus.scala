...
class KmeansPlus (val k:Int = 3,
  ...
  def _init_random_centroids(data : List[DenseVector[Double]]):Unit = {
    ...
    val data_copy = data
    val res1 = rng.shuffle(data_copy).take(1).apply(0)
    centroids.append(res1)
    while (centroids.size < k){
      centroids.append(_choose_next_center(data))
    }
  }

  def _choose_next_center(data : List[DenseVector[Double]]):DenseVector[Double] = {
    val distance:ArrayBuffer[DenseVector[Double]] = new ArrayBuffer[DenseVector[Double]]()
    for (j <- centroids.indices) {
      val d = centroids.map(f => MatrixUtils.euclidean_distance(f, centroids(j)))
